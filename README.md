...
A CLI tool that can save, load, and edit a `settings.toml` file. It must handle errors gracefully (e.g., using `anyhow` if the file is missing) and using `log`+`simplelog` to document all the processes so I can see exactly what the application is doing, and where it might be failing.
...
Extra crate for actual project: [directories](https://crates.io/crates/directories)
